...
def list_dir(path: str = ".") -> str:
    """High-signal directory listing that distinguishes files and folders."""
    try:
        # Use ls -F to mark directories with /
        cmd = f"ls -F '{path}'"
...
def google_search(query: str) -> str:
    try:
        ...
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=30.0)
        if result.returncode != 0: return "Error: Search failed."
...
TOOLS_METADATA = [
    ...
    {"type": "function", "function": {"name": "google_search", "description": "Search the internet.", "parameters": {"type": "object", "properties": {"query": {"type": "string"}}, "required": ["query"]}}},
