...
"""
    ...
    def get_system_prompt(self, role: str = "coordinator") -> str:
        ...
        if self.memory_context:
            memory_instr = f"\n# Memory & User Instructions\n{self.memory_context}\nIMPORTANT: Adhere to these instructions exactly as written."
        
        base = self.master_system_prompt.format(memory_instruction=memory_instr)
        if role == "verification":
            base += "\n\n# VERIFICATION ROLE: Adversarial testing. End with VERDICT: PASS or FAIL."
        elif role == "explore":
            base += "\n\n# EXPLORE ROLE: Read-only research. Do NOT modify files."
        elif role == "worker":
            base += "\n\n# WORKER ROLE: Implementation. Self-verify your work."
        return base
    ...
    def chat_with_tools(self, messages: List[Dict[str, str]], role: str = "coordinator", stream_callback=None):
        ...
        while iteration < MAX_ITERATIONS:
            ...
            if message.get("content") and stream_callback: stream_callback(message["content"])
            if not message.get("tool_calls"): return message.get("content", "")
